...
class OptProb:
    ...
    def eval_validation_fitness(self, state):
        if len(state) != self.length:
            raise Exception("state length must match problem length")

        fitness = self.maximize*self.fitness_fn.evaluateFitnessValidation(state)
    ...
    def eval_mate_probs(self):
        ...
        if np.sum(pop_fitness) == 0:
            ...
            self.mate_probs = np.exp(pop_fitness)/np.sum(np.exp(pop_fitness))
    ...
    def set_state(self, new_state):
        ...
        self.train_fitness.append(self.maximize * self.fitness)
        # self.val_fitness.append(self.maximize * self.eval_validation_fitness(new_state))

    def plot_train_val(self):
        x_axis = range(len(self.train_fitness))
        plt.plot(x_axis, self.train_fitness)
        plt.plot(x_axis, self.val_fitness)
        plt.legend(["train fitness", "val fitness"])
        plt.show()
...
class ContinuousOpt(OptProb):
    ...
    def larger_neighbor(self, state):
        leftNeighbor = np.copy(state)
        rightNeighbor = np.copy(state)
