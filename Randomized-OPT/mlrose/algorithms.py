...
import numpy as np
from .decay import GeomDecay, ArithDecay, ExpDecay
import collections
import util
...
def random_hill_climb(problem, max_attempts=10, max_iters=np.inf, restarts=0,
    ...
    for k in range(restarts + 1):
        ...
        while (attempts < max_attempts) and (iters < max_iters):
            ...
            if next_fitness > curIterBestFitness:
                curIterBestFitness = next_fitness

            # If best neighbor is an improvement,
            # move to that state and reset attempts counter
            if next_fitness > problem.get_fitness():
                problem.set_state(next_state)
                fitnessResult.append(-next_fitness)
                attempts = 0
            else:
                attempts += 1

            print ("{}: {}".format(iters, problem.get_maximize() * problem.get_fitness()))

        # Update best state and best fitness
        if problem.get_fitness() > best_fitness:
            best_fitness = problem.get_fitness()
            best_state = problem.get_state()
        # util.plot_graph(fitnessResult, range(len(fitnessResult)), "loss (-fitness)", "valid iteration")

        restart_best_fitness.append(-curIterBestFitness)
        print ("restart: {}  best_fitness: {}".format(k, curIterBestFitness))

    best_fitness = problem.get_maximize()*best_fitness
    # util.plot_graph(restart_best_fitness, range(len(restart_best_fitness)), "loss (-fitness)", "restart")
    return best_state, best_fitness
...
def simulated_annealing(problem, schedule=GeomDecay(), max_attempts=10,
    ...
    globalBestState = problem.get_state()
    for decayName, schedule in [("Geom", GeomDecay())]:
        #, ("Exp", ExpDecay()), ("Arith", ArithDecay())]:
        #print ("currently visited: {}".format(decayName))
        ...
        while (attempts < max_attempts) and (iters < max_iters):
            ...
            if temp == 0:
                ...
                if (delta_e > 0) or (np.random.uniform() < prob):
                    problem.set_state(next_state)
                    data[decayName].append(-problem.get_fitness())
                    attempts = 0
        ...
        if best_fitness > problem.eval_fitness(globalBestState):
            globalBestState = best_state

        # problem.plot_train_val()

    # for key, l in data.items():
    #     print (key)
    #     plt.plot(range(len(l)), l)
    # plt.legend(["Geom", "Exp", "Arith"])
    # plt.show()

    return globalBestState, problem.eval_fitness(globalBestState)


def genetic_alg(problem, pop_size=200, mutation_prob=0.01, max_attempts=10,
                max_iters=np.inf):
    ...
    while (attempts < max_attempts) and (iters < max_iters):
        ...
        for _ in range(pop_size):
            # Select parents
            mate_probability = problem.get_mate_probs()
            # print (sum(mate_probability))
        ...
        best_fitness.append(problem.get_maximize() * next_fitness)
        avg_fitness.append(problem.get_maximize()  * np.average(problem.pop_fitness))
        print("{}: {}".format(iters, problem.get_maximize() * problem.get_fitness()))
    ...
    # plt.ylabel("fitenss")
    # plt.legend(["best fitness", "average_fitness"])
    #
    # plt.show()
...
def mimic(problem, pop_size=200, keep_pct=0.3, max_attempts=10,
          max_iters=np.inf):
